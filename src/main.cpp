...
public:
  ...
  bool getPoint(uint16_t &x, uint16_t &y, bool &pressed)
  {
    ...
    if (TOUCH_INVERT_X)
      tx = (_xMax > 0 ? (_xMax - 1 - tx) : tx);
    if (TOUCH_INVERT_Y)
      ty = (_yMax > 0 ? (_yMax - 1 - ty) : ty);
...
private:
  ...
  {
    size_t got = _w->requestFrom((int)_addr, (int)n);
    if (got != n)
      return false;
    for (size_t i = 0; i < n; i++)
      p[i] = _w->read();
...
static void alloc_lvgl_buffers()
{
  ...
  for (int l : {40, 30, 20, 10})
    if (try_alloc(l, true))
      return;
  for (int l : {20, 10})
    if (try_alloc(l, false))
      return;
  // Last resort tiny single buffer
  int l = 8;
  size_t px = (size_t)HOR_RES * l;
...
static void sensor_timer_cb(lv_timer_t *t)
{
  ...
  {
    char buf[64];
    snprintf(buf, sizeof(buf), "Chamber: %.1f°C   RH: %.0f%%", tc, rh);
    if (lbl_readout)
      lv_label_set_text(lbl_readout, buf);
  }
  else
  {
    if (lbl_readout)
  ...
  {
    if (lbl_rtd)
      lv_label_set_text(lbl_rtd, "PT100: ERR");
  }
  else
  {
    char b2[32];
    snprintf(b2, sizeof(b2), "PT100: %.1f°C", rtdTemp);
    if (lbl_rtd)
      lv_label_set_text(lbl_rtd, b2);
  }
}
...
void setup()
{
  ...
  if (!aht.begin(Wire))
    ...
    Serial.println("[AHT30] init OK @ 0x38");
  if (!rtd.begin(MAX31865_3WIRE))
    Serial.println("[MAX31865] init failed");
  else
    Serial.println("[MAX31865] init OK (3-wire)");
